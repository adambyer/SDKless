...
class Authentication {
	 ...
	 */
	public function prepare_auth_step($step, $incoming_params) {
				...
				if (!empty($this->config->settings->endpoints->{$step->endpoint}->merge_maps)) {
					foreach ($this->config->settings->endpoints->{$step->endpoint}->merge_maps as $incoming_key => $merge_key) {
						if (!empty($incoming_params[$incoming_key]))
							$this->global_vars['merge'][$merge_key] = $incoming_params[$incoming_key];
					}
	 ...
	 **/
	public function setup_oauth_header() {
		...
		$this->global_vars['merge']['OAUTH-HEADER-PARAMS'] = implode(', ', $oauth_header_array);
		$this->config->apply_global_vars($this->global_vars);
	}
