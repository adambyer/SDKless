...
class SDKless {
	...
	public function __construct($api_name = null, $global_vars = array()) {
		...
		$this->config->json = file_get_contents("$core_dir/config/{$this->api_name}.json");
		$this->config->json_custom = @file_get_contents("$current_dir/config/{$this->api_name}.custom.json");
		$this->config->setup();
	...
	public function authenticate($step_id, $params = null, &$done) {
		...
		$this->auth->prepare_auth_step($step, $params);
		...
		switch ($step->type) {
			...
			case "endpoint":
				...
				return array_merge($params, (array)$output);
				break;
			default:
	...
	public function go($endpoint_name, $endpoint_vars = array(), $local_vars = array()) {
		...
		// loop for paging
		while (true) {
			$response = $this->response->get($this->config);
